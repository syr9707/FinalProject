		...
		<section class="section">
			<form id="mypage_check_form" method="post" action="<c:url value='/mypage_check'/>">
				<div class="section_wrap">
                    ...
                    <div><input type="password" class="pw" name="pw" placeholder="비밀번호 입력"></div>
                    <div class="error red" id="pw_chk_error">비밀번호가 일치하지 않습니다.</div>
                    <div><input type="button" class="check bold txt_20 bg_y" name="check" value="확인"></div>
				</div>
			</form>
		</section>
		<form id="to_mypage_update" method="post" action="/mypage_update">
			<input type="hidden" name="chk" id="chk" value="${chk }">
		</form>
		<form id="to_delete_memberInfo" method="post" action="/delete_memberInfo">
			<input type="hidden" name="chk" id="chk2" value="${chk }">
		</form>
		<!-- bottom.jsp -->
